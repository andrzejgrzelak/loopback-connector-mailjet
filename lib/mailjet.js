...
//email from param
            ...
            if (_.isString(options.from)) {
                emailData.From = {"Email": options.from};
            } else if (_.isObject(options.from)) {
                emailData.From = {"Name": options.from.name};
                if (options.from.hasOwnProperty('address')) {
                    emailData.From.Email = options.from.address;
                } else if (options.from.hasOwnProperty('email')) {
                    emailData.From.Email = options.from.email;
                }
                delete options.from;
            } else {
                if (mailjetSettings && mailjetSettings.default) {
                    emailData.From = {};
                    if (mailjetSettings.default.fromName) {
                        emailData.From.Name = mailjetSettings.default.fromName;
                    }
                    if (mailjetSettings.default.fromEmail) {
                        emailData.From.Email = mailjetSettings.default.fromEmail || '';
                    }
                }
        ...
        if (options.html) {
            if (mailjetSettings.apiVersion === 'v3') {
                ...
                emailData["HTMLPart"] = options.html;
            }
            delete options.html;
        }
        ...
        if (options.text) {
            if (mailjetSettings.apiVersion === 'v3') {
                ...
                emailData["TextPart"] = options.text;
            }
            delete options.text;
        }
