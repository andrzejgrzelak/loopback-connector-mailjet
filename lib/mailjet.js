...
const MailjetConnector = function MailjetConnector(settings) {
    ...
    const is_valid_version = AVAILABLE_MAILJET_VERSIONS.indexOf(settings.version) > -1;
    ...
    assert(!settings.version || is_valid_version, 'invalid API version, avaiable versions: ' + AVAILABLE_MAILJET_VERSIONS.join(','));

    settings.version = settings.version | 'v3';
